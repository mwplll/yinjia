/**
  ...
  */
define(["avalon",
    "text!./avalon.tooltip.html",
    "../position/avalon.position.js",
    "css!./avalon.tooltip.css"], function(avalon, template) {
        ...
        var vmodel = avalon.define(data.tooltipId, function(vm) {
            ...
            vm.show = function(elem) {
                ...
                if(elem) {
                    ...
                    if(elem.nodeName) {
                        if(tipElem.position().top > atEle.position().top + elem.offsetHeight
                            && vmodel.arrClass == "bottom") {
                            if(vmodel.posHack == 'bottom'){
                                // hack 位置计算错误
                                console.log()
                            }else{
                                vmodel.arrClass = "top"
